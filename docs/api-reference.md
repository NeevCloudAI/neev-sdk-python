...
| `update(id, params, org_id=None, project_id=None)` | `Sandbox` | Resizes cpu/memory in place on the running sandbox; sizes left out of `resources` are unchanged, and disk is not resizable this way. Rejects a resize-nothing patch locally. |
| `pause(id, org_id=None, project_id=None)` | `Sandbox` | Scales a sandbox to 0 replicas (Paused state), always capturing the full snapshot (root filesystem, process memory, workspace). |
| `resume(id, org_id=None, project_id=None)` | `Sandbox` | Scales a sandbox back to 1 replica toward Ready, restoring from the snapshot the pause captured. |
...
| `update(params)` | method — resizes cpu/memory in place |
...
| `client.sandboxes.update(id, params)` | `sandbox = client.sandboxes.update(sandbox_id, {"resources": {"cpu": 2, "memory_gb": 4}})` | `sandbox = await client.sandboxes.update(sandbox_id, {"resources": {...}})` | [sandbox_lifecycle_controller.py](../examples/sandbox_lifecycle_controller.py) |
...
| `sandbox.update(params)` | `sandbox.update({"resources": {"cpu": 2, "memory_gb": 4}})` | `await sandbox.update({"resources": {...}})` | — |
